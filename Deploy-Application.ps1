...
Try {
	...
	[string]$appName = 'STK Network License'
	[string]$appVersion = '12.4.0'
	[string]$appArch = 'x64'
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.0.15'
	[string]$appScriptDate = '06/24/2022'
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		Write-Log -Message "Checking to see if application exists..." -Source 'Pre-Installation' -LogType 'CMTrace'
		If (Test-Path "$envProgramFiles\AGI\STK 12\bin\AgUiApplication.exe") {
			Write-Log -Message "Checking to see if license directory exists..." -Source 'Pre-Installation' -LogType 'CMTrace'
			If (-not (Test-Path "$envProgramFiles\AGI\STK 12\Shared Files\Licensing")) {
				...
				New-Item -Path "$envProgramFiles\AGI\STK 12\Shared Files\Licensing" -ItemType "directory"
			}
			Write-Log -Message "Importing license file..." -Source 'Pre-Installation' -LogType 'CMTrace'
			Copy-Item -Path "$dirSupportFiles\ansyslmd.ini" -Destination "$envProgramFiles\AGI\STK 12\Shared Files\Licensing\ansyslmd.ini"
			If (Test-Path "$envProgramFiles\AGI\STK 12\Shared Files\Licensing\ansyslmd.ini") {
				Write-Log -Message "License file imported successfully." -Source 'Pre-Installation' -LogType 'CMTrace'
			}
			Else {
				Write-Log -Message "License file import failed." -Source 'Pre-Installation' -LogType 'CMTrace'
			}
		}
		Else {
			Write-Log -Message "Application not detected." -Source 'Pre-Installation' -LogType 'CMTrace'
		}
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		If (Test-Path "$envProgramFiles\AGI\STK 12\Shared Files\Licensing\ansyslmd.ini") {
			Remove-Item -Path "$envProgramFiles\AGI\STK 12\Shared Files\Licensing\ansyslmd.ini" -Force
			Write-Log -Message "License file successfully removed." -Source 'Pre-Installation' -LogType 'CMTrace'
		}
		Else {
			Write-Log -Message "License file not detected." -Source 'Pre-Installation' -LogType 'CMTrace'
		}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUB87zWJjTKDHVzA0E5rZx8eba
# tbygghHXMIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# MRYEFATXDlAlYSypUIXotemBjIjxOg2UMA0GCSqGSIb3DQEBAQUABIIBgFJI1M+s
# djxZGzrqHS1d1BOXQqPWlaO9vKqMKmxWArLEi4cI7bX6/DdZPHXDJyS7t7GpA8Fi
# 4gC7f2DTAAMzsQzfNwaJMSQt4ntxvOtmlGtOb3KWpBMJ/31cCQogsbW/usYTsxFa
# 2WEyNpsldQCWnYHAZUW9AojubkTpWP7l7U81LH7Co+XYRgxgJoR0iVR+CgsSibaT
# LhPBL4entApDdJtzjjTUwQWF9vkQ2lIAhnD/OsrnbPFzRITSClAyABjEKOzCrQ7g
# NQP5iF4I7FKPLOm2jKN/FnnZgU1zodIiAsLQmDq6t6tT0Ldzt3RC7+Zq19cugrRL
# 0jSZPq06K17t2SrL/msMzGlULmftQmEX195CmKf/M796wrYe83lmProCJjPODE4l
# LtlEAvvOyMz//NS9dgO7gphbGOvzPUMxSajkUMgwUVfx72KvD4+ELVA7zrZ0Qale
# 9x9QM1g2Uc+0LPgtxn9f+rzqJFMovFEwsx5qPf2Q+7CVUc0JsB1HbmQPUg==
